...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete'])) {
        ...
        if ($stmt->execute()) {
            $success_message = "Skill entry deleted successfully.";
        } else {
    ...
    // --- Add/Update Action ---
        ...
        if ($id) { // Update
            $stmt = $conn->prepare("UPDATE skills SET name = ?, category = ?, level = ? WHERE id = ?");
            $stmt->bind_param("ssii", $name, $category, $level, $id);
            $action = "updated";
        } else { // Insert
            $stmt = $conn->prepare("INSERT INTO skills (name, category, level) VALUES (?, ?, ?)");
            $stmt->bind_param("ssi", $name, $category, $level);
            $action = "added";
        }

        if ($stmt->execute()) {
            $success_message = "Skill entry {$action} successfully.";
        } else {
...
// Fetch all skill entries to display
$skills_entries = [];
$result = $conn->query("SELECT * FROM skills ORDER BY category, name");
if ($result) {
    $skills_entries = $result->fetch_all(MYSQLI_ASSOC);
}
...
<body>
    ...
    <div class="container">
        ...
        <form action="manage_skills.php" method="post">
            <h2><?php echo $edit_item ? 'Edit' : 'Add New'; ?> Skill Entry</h2>
            ...
            <select id="category" name="category" required>
                <option value="hard" <?php echo (isset($edit_item['category']) && $edit_item['category'] == 'hard') ? 'selected' : ''; ?>>Hard Skill</option>
                <option value="soft" <?php echo (isset($edit_item['category']) && $edit_item['category'] == 'soft') ? 'selected' : ''; ?>>Soft Skill</option>
            </select>

            <label for="level">Proficiency Level (0-100)</label>
            <input type="number" id="level" name="level" min="0" max="100" value="<?php echo htmlspecialchars($edit_item['level'] ?? '80'); ?>" required>

            <button type="submit"><?php echo $edit_item ? 'Update' : 'Add'; ?> Skill</button>
        </form>
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($skills_entries)): ?>
                    <tr><td colspan="4">No skills found.</td></tr>
                <?php else: ?>
                    <?php foreach ($skills_entries as $entry): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($entry['name']); ?></td>
                        <td><?php echo htmlspecialchars(ucfirst($entry['category'])); ?></td>
                        <td><?php echo htmlspecialchars($entry['level']); ?>%</td>
                        <td class="actions">
                            <a href="manage_skills.php?edit=<?php echo $entry['id']; ?>">Edit</a>
                            <form action="manage_skills.php" method="post" style="display:inline;">
                                <input type="hidden" name="id" value="<?php echo $entry['id']; ?>">
                                <button type="submit" name="delete" class="delete" onclick="return confirm('Are you sure you want to delete this skill?');">Delete</button>
                            </form>
                        </td>
                    </tr>
                    <?php endforeach; ?>
