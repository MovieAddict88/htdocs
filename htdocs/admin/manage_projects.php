...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete'])) {
        ...
        if ($stmt->execute()) {
            $success_message = "Project entry deleted successfully.";
        } else {
    ...
    // --- Add/Update Action ---
        ...
        if ($id) { // Update
            $stmt = $conn->prepare("UPDATE projects SET title = ?, description = ?, category = ?, image_url = ?, video_url = ?, external_link = ? WHERE id = ?");
            $stmt->bind_param("ssssssi", $title, $description, $category, $image_url, $video_url, $external_link, $id);
            $action = "updated";
        } else { // Insert
            $stmt = $conn->prepare("INSERT INTO projects (title, description, category, image_url, video_url, external_link) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $title, $description, $category, $image_url, $video_url, $external_link);
            $action = "added";
        }

        if ($stmt->execute()) {
            $success_message = "Project entry {$action} successfully.";
        } else {
...
// Fetch all project entries to display
$project_entries = [];
$result = $conn->query("SELECT * FROM projects ORDER BY title");
if ($result) {
    $project_entries = $result->fetch_all(MYSQLI_ASSOC);
}
...
<body>
    ...
    <div class="container">
        ...
        <form action="manage_projects.php" method="post">
            <h2><?php echo $edit_item ? 'Edit' : 'Add New'; ?> Project</h2>
            ...
            <textarea id="description" name="description" rows="5"><?php echo htmlspecialchars($edit_item['description'] ?? ''); ?></textarea>

            <label for="image_url">Image URL(s) (comma-separated)</label>
            <input type="text" id="image_url" name="image_url" value="<?php echo htmlspecialchars($edit_item['image_url'] ?? ''); ?>">

            <label for="video_url">Video URL (e.g., YouTube embed)</label>
            <input type="text" id="video_url" name="video_url" value="<?php echo htmlspecialchars($edit_item['video_url'] ?? ''); ?>">

            <label for="external_link">External Link (e.g., Live Demo)</label>
            <input type="text" id="external_link" name="external_link" value="<?php echo htmlspecialchars($edit_item['external_link'] ?? ''); ?>">

            <button type="submit"><?php echo $edit_item ? 'Update' : 'Add'; ?> Project</button>
        </form>
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            <a href="manage_projects.php?edit=<?php echo $entry['id']; ?>">Edit</a>
                            <form action="manage_projects.php" method="post" style="display:inline;">
                                <input type="hidden" name="id" value="<?php echo $entry['id']; ?>">
                                <button type="submit" name="delete" class="delete" onclick="return confirm('Are you sure you want to delete this project?');">Delete</button>
                            </form>
                        </td>
                    </tr>
                    <?php endforeach; ?>
